...
\centering
\vspace*{3.2cm}
\includegraphics[width=5.0cm]{assets/bc_logo.pdf}\\[2.4cm]
...
{\color{bcgrey}Revision 3 --- 4 August 2026}\\[0.5cm]
...
\textbf{Revision 3} restates why the OU sleeve is in the book. Revision 1 justified it as a
hedge on the strength of a $0.09$ return correlation with the Bayesian sleeve. That figure does
not survive re-measurement: on the live book the correlation is $0.57$, and conditional on both
sleeves actually holding stock it is $0.90$ or higher. A number near $0.09$ is what an idle
sleeve produces, not an independent one. Nor does the sleeve reduce drawdown where it matters:
book-level drawdown is $39$--$40\%$ at every possible split. \S\ref{sec:hedge} sets out what the
sleeve does instead --- diversify \emph{timing}, and, since the correction below, earn more than
the Bayesian sleeve does.

\textbf{Revision 2} corrected one equation. The OU sleeve's uncertainty scale, previously the
...
\subsection{What the sleeve actually diversifies}\label{sec:hedge}
...
one bidding for the washed-out bounce. That mechanism is real and visible in the bids themselves.

What it does \emph{not} produce is independent P\&L. Measured on the live book at the deployed
split, the two sleeves' daily equity returns correlate at $\mathbf{0.57}$ on the superseded sigma
and $0.59$ on the corrected one, ranging $0.20$ to $0.89$ across names. Revision~1 reported
$0.09$; that figure could not be reproduced under any configuration we tested, the closest being
$0.32$ at its own stated settings.

\begin{fixbox}{Why a correlation near zero is not evidence of a hedge}
Sweeping the OU buffer on NVDA at $W=120$ shows what the statistic actually tracks:

\begin{center}\small
\begin{tabular}{rrrrr}
\toprule
\textbf{buffer $k$} & \textbf{OU invested} & \textbf{both invested} & \textbf{corr (all days)} &
\textbf{corr (both)}\\
\midrule
0.25 & 47\% & 37\% & 0.49 & 0.94\\
0.50 & 24\% & 18\% & 0.32 & 0.93\\
1.00 & 7\%  & 7\%  & 0.16 & 0.53\\
1.50 & 0\%  & 0\%  & 0.21 & ---\\
2.00 & 0\%  & 0\%  & 0.05 & --- (no overlapping days)\\
\bottomrule
\end{tabular}
\end{center}

The correlation falls toward zero exactly as the sleeve stops trading. At $k\ge1.5$ it is never
invested, there are no overlapping observations, and the statistic reads $0.05$ while measuring
nothing at all.

\textbf{Conditional on both sleeves actually holding stock, the correlation is $0.90$ to $0.97$ in
every configuration tested.} Two sleeves long the same name on the same day are the same trade.
What diversifies is \emph{when} each is invested, not what each believes; the low all-days figure
is cash periods diluting the series.
\end{fixbox}

\subsection{What the correction did to the hedge}

Residual sigma moves the OU bid closer to the market, so the sleeve trades far more. Across the
five daily names it is now invested $48\%$ of days against $31\%$ before, and overlaps the
Bayesian sleeve $40\%$ of days against $27\%$. The correction therefore buys return at the cost of
exactly the timing separation the sleeve was included for. We record the trade-off rather than
resolve it, because the return gain is large and measured while the timing benefit turns out to be
small.
...
that the tilt was compensating for the mis-specification. It was: measured directly with both
configurations frozen, a $75\%$ share beat $50/50$ in 11 of 15 walk-forward folds on the old
sleeve but only 7 of 15 on the corrected one. The tilt was worth something against a
mis-specified OU sleeve and is worth nothing against a correct one, so the book now runs a flat
$50/50$.

Because the sleeve's justification was risk rather than return, the split was then tested on
risk-adjusted objectives as well. Out-of-sample Sharpe on the equal-weighted book, with the OU
buffer re-fitted inside each fold on training data only, is $2.56$ at $0\%$ Bayes, $2.60$ at
$25\%$, $2.58$ at $50\%$, $2.51$ at $75\%$ and $2.40$ at $100\%$. The spread across the middle
three is $0.04$: on risk-adjusted terms the split is very nearly irrelevant, and only the pure-Bayes
end is clearly worse. Choosing the share adaptively remains unsupported on every objective, winning
1 fold in 3.

\begin{keybox}{A near miss worth recording}
Our first pass at this showed return, Sharpe and Calmar all improving monotonically as the Bayes
share fell, with pure OU best on all three --- three independent objectives agreeing, which is
what a real effect looks like. It was an artefact. The OU buffer had been fitted at $0\%$ Bayes on
the full sample, so the pure-OU configuration was precisely the one whose parameter had been tuned
to it. Re-fitting the buffer inside each fold flattened the curve to the $0.04$ spread above.

The lesson generalises beyond this test: when a comparison involves a fitted parameter, ask which
configuration it was fitted to before believing the ranking.
\end{keybox}
...
fades deviations from a slow mean and, at a long lookback, is invested at different times. Its
place in the book has been restated: it is not the near-independent hedge Revision~1 described ---
the sleeves correlate $0.57$, and $0.90$ or more whenever both are actually holding, while
book-level drawdown is unchanged by the split --- but on the corrected scale it out-earns the
Bayesian sleeve, $88\%$ against $71\%$, with slightly lower drawdown. It is kept because it makes
money, not because it hedges.
