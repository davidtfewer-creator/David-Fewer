...
\toprule
 & full sample & planning & trades/yr & AI $\beta$ & corr to five-book \\
\midrule
TSM  &  56\% &  55\% &  92 & 0.69 & 0.47 \\
VRT  &  58\% &  65\% &  74 & 1.19 & 0.48 \\
VST  &  50\% &  60\% &  78 & 0.98 & 0.35 \\
RKLB & 161\% & 156\% & 118 & 0.79 & 0.94 \\
MU   &  65\% &  61\% &  57 & 1.14 & 0.59 \\
GM   &  50\% &  33\% &  61 & 0.17 & 0.24 \\
VLO  &  64\% &  35\% &  35 & 0.11 & 0.04 \\
CF   &  47\% &  41\% &  27 & $-0.01$ & $-0.01$ \\
MRVL &  85\% &  78\% &  57 & 0.95 & 0.22 \\
\midrule
\textbf{average / total} & \textbf{71\%} & \textbf{65\%} & \textbf{599} & \textbf{0.67} & \textbf{0.37} \\
\bottomrule
\end{tabular}
\end{center}

\tabnote{\textbf{Full sample} is the measured through-cycle verified return of the
deployed/reference vector --- it carries that vector's fit lookahead and is a
measurement, not a target. \textbf{Planning} is the figure to plan on: for the
incumbents the established planning basis (full-sample less the measured
out-of-sample haircut, per the book specification); for the additions the June
unseen-window medians, which the new tested numbers are consistent with. On
planning figures the nine-name book runs $\approx$\textbf{65\%/yr}; the measured
held construction over the sample gives 72.1\%/yr (the pooled live loop, 75.8\%). Trades/yr is the average
verified buy count per year; the bottom row is the book's \emph{total},
$\approx$599 fills a year, about 2.4 per session across the eighteen sleeves.
Correlations are measured against the \emph{current five-name} book on the tested
half, so the incumbents' values partly reflect self-correlation; the candidates'
values are the clean read.}
...
\tabnote{The planning figures in \S7 are left unchanged and should continue to
anchor expectations --- they are now known to be conservative by roughly this
margin. The correction is an execution fact, not an edge claim: every A-vs-B
comparison behind this memo's decisions measured both sides on the same
(conservative) convention, so no verdict changes. The effect concentrates in
the long-hold AI names (MU, MRVL, VRT), whose seats it strengthens.}
