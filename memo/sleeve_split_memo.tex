...
time. The durable, regime-robust actions are per-name: \textbf{RKLB $\to$ OU-only} and
\textbf{AVGO $\to$ Bayes-heavy}; most other names lean OU.}}
\end{titlepage}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \kpi{The two firm per-name calls.}{} \textbf{RKLB} should be OU-only (OU: 569\% at a 9\%
        drawdown vs Bayes: 604\% at a 50\% drawdown). \textbf{AVGO} should be Bayes-heavy --- the
        one name whose OU sleeve draws down \emph{more} than its Bayes sleeve.
\end{itemize}
...
two exceptions that hold under either criterion: RKLB (OU-only) and AVGO (Bayes-heavy).
Bayes\,\% is the fraction of the name's capital in the Bayes tranche; OU\,\% is the remainder.}
\end{table}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item \kpi{Efficiency first (recommended):}{} use the \textbf{Sharpe-opt} column ---
        NVDA~20, TSM~10, TSLA~15, VRT~20, VST~15, AVGO~55, PLTR~5, RKLB~0, SOFI~10, SPOT~10.
  \item \kpi{Return-aware:}{} use the \textbf{Calmar-opt} column if you want capital working
        harder and can accept more drawdown --- the differences are VRT~45, TSLA~25, PLTR~25,
        AVGO~100, SPOT~55; the rest unchanged.
\end{itemize}

Regardless of column, make the two firm calls first: \textbf{RKLB $\to$ 0\% Bayes (OU-only)} and
\textbf{AVGO $\to$ Bayes-heavy}. They are the largest, most robust corrections to the uniform
50/50 and hold under both criteria.
