...
\subsection{What it costs in the bull (2024--26, measured)}

\begin{center}
\begin{tabular}{lccc}
\toprule
 & without gate & with gate & cost \\
\midrule
book total return (2.35 yrs) & $+238.7\%$ & $+183.8\%$ & \\
annualised                   & $\approx 68\%$/yr & $\approx 56\%$/yr & \bad{$\approx 12$pp/yr} \\
on a €9m book                & ends $\approx$ €30.5m & ends $\approx$ €25.5m & €5m forgone \\
\bottomrule
\end{tabular}
\end{center}

The cost is concentrated, not spread: near-free on TSM, VRT, RKLB, MU, VLO and GM
(3--16\% of days gated), with nearly the whole bill from VST ($49.6\% \rightarrow
22.6\%$/yr), MRVL ($85.1\% \rightarrow 56.3\%$) and CF ($46.9\% \rightarrow
31.0\%$) --- names that spent long spells \emph{below} their line while bottoming
and rallying. That is the gate's inherent weakness: it cannot tell ``recovering
off the low'' from ``breaking down'' until price crosses back over, and the
model's fattest fills live in exactly those cheap, hated stretches.
...
\textbf{200dma gate}             & \good{$-2.6\%$} & \good{17.4\%} & \good{$+7.3\%$} & 17.4\% & $\sim$12pp/yr \\
gate $+$ breaker                 & $-10.2\%$ & 20.9\% & $-1.1\%$ & 20.9\% & $\sim$16pp/yr \\
gate $+$ breaker $+$ stop $-25\%$& $-9.9\%$  & 20.4\% & $-0.9\%$ & 20.4\% & $\sim$16pp/yr \\
\bottomrule
\end{tabular}
\end{center}

\tabnote{Pooled book, equal weights, €9m, verified fills. Bull costs measured on
the 2024--26 sample. Note the anti-synergy: the breaker stacked on the gate makes
the bear \emph{worse} because it half-sizes the diversifier entries the gate has
just concentrated capital into.}
...
The two-sided price of the gate: \textbf{$\sim$12pp/yr of premium in bull years;
$\sim$19pp and 11pp of drawdown returned in the bear year.} In pure
expected-value terms that is a losing trade --- US bear markets arrive roughly
twice a decade, and at that frequency a 12pp annual premium buys back its cost
only once every several years. Whether to pay it is therefore not a
return-optimisation question but a risk one, and three considerations argue for
paying:

\begin{itemize}
\item \textbf{Drawdown control compounds.} A book that loses 2.6\% instead of
21.9\% needs no recovery year; the gated path's worst drawdown (17.4\%) is
comfortably inside what the live book has already survived (29.2\% in April
2025).
\item \textbf{The premium is concentrated and known} (three names' recovery
rallies); the payout arrives precisely when capital is hardest to replace.
\item \textbf{The alternative defences are worse}, and the volatility-based ones
are proven counterproductive --- the overlays paper showed the model \emph{earns}
from stress; only direction can be gated.
\end{itemize}
...
\begin{tcolorbox}[colback=bcgold!8, colframe=bcgold, boxrule=0.8pt,
  title={\bfseries Adopt}, coltitle=bcnavy]
\textbf{The 200dma gate, alone,} on all nine names: no new bids in a name whose
previous close sits below its trailing 200-day average; exits untouched; freed
capital pools as normal. Implementation is one column and one flag per Model
sheet ($\approx$ an hour of workbook work). Optionally, retain the circuit
breaker as a \emph{fast-crash-only} satellite (trip 15\%, action half-size, reset
on market signal or drawdown recovery --- never full stand-down with an equity
reset), accepting that it will occasionally cost a V-recovery. Skip the
per-stock stop.
\end{tcolorbox}
...
(breaker, price-stop and gate support). Findings recorded in HANDOVER
\S3.13. Companion paper: \emph{Three Volatility Overlays, Tested and Rejected}
(14 Aug 2026).}
