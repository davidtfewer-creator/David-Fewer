...
names); the re-run is queued on that data. The unverified expectation, stated
for the record and to be tested: AVGO's public-record 2022 drawdown (roughly
$-40\%$) is materially shallower than the $-69\%$ the replay charged the
fourth slot (\S1), so the unprotected 2022 result should soften, while the
gated result should remain near flat since the gate's save came from the
diversifiers.
