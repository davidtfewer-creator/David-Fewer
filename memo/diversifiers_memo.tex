...
Assessed as decorrelating additions to the AI-heavy book. The genuine diversifiers that also
survive out of sample are \textbf{MRNA, OXY, DVN and FSLR}. \textbf{SHOP} and \textbf{ARM} add
return but little decorrelation; \textbf{COIN} is high-octane, size down. \textbf{Reject MSTR
and HOOD} --- their large in-sample returns collapse to \emph{losses} on unseen data.}}
\end{titlepage}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item \kpi{Add --- genuine diversifiers, survive OOS:}{} \textbf{MRNA} (biotech; strongest
        OOS, decorrelated, good internal hedge), \textbf{OXY} (energy; lowest drawdown, best
        decorrelation, OOS $>$ train --- the ballast name), \textbf{FSLR} (solar; decorrelated,
        holds OOS), \textbf{DVN} (energy; superb Sharpe and drawdown, decorrelated, though its
        honest OOS return is modest).
...
\textbf{Best fit for the stated goal} (diluting the AI concentration): \textbf{MRNA, OXY, FSLR,
DVN} --- biotech, energy and solar, all decorrelated from the book and all surviving the holdout.

\textbf{Caveats.} (1) Fitted parameters are freshly optimised; treat full-sample figures as a
ceiling and the OOS column as the trustworthy number. (2) All history sits in a strong bull
regime; a 60/40 holdout is one split, not a multi-fold walk-forward --- a fuller walk-forward on
the shortlist is the natural next step before committing capital. (3) Market cap, P/E and \$
turnover await a volume column and fundamentals.
