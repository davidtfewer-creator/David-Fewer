...
{\color{bcblue}\LARGE Revision 5 --- Corrected Sleeve, 50/50 Split}\\[0.5cm]
...
{\color{bcgrey} 4 August 2026 --- Revision 5, supersedes Revision 4}\\[1.6cm]
...
\textbf{Revision 5 supersedes Revision 4}, which was issued before the planning figures had
been re-derived on the corrected sleeve. Doing so changed two things. The planning figures rise
materially, and every name now clears the 50\% floor. And the Bayes tilt reverses: measured
directly against a fixed 50/50, the 75\% share won 11 of 15 walk-forward folds on the old sleeve
but only 7 of 15 on the corrected one. \textbf{The split moves to 50/50.} Revision 4's statement
that the split stays at 75\% rested on a different question --- whether the share should be chosen
adaptively, which is still answered no --- and did not test the fixed comparison the planning
basis depends on. Section 9 sets both out.

\textbf{Revision 4 corrected the OU sleeve's uncertainty scale on the five daily names.} It was
the standard deviation of the closes about the window mean, which on a trending name measures the
trend rather than the innovation and inflated the entry discount exactly when the sleeve should
have been working. It is now the standard deviation of the fitted AR(1) residuals. This is a
specification correction, not a re-fit, and it is the only change to a signal that has survived
walk-forward here. Section 9 sets it out. The weekly names, the parameters, the allocation and
the Bayes share are all unchanged.

\textbf{Revision 3 froze this configuration.} The only change since Revision 2 is a 26-week
...
\textbf{Weight}\\
\midrule
\rowcolor{bckeep} RKLB & Daily hybrid   & 158\% & 154\% & 158\% & 14.29\%\\
AVGO & Weekly Monday  & 91\%  & 96\%  & 60\%  & 14.29\%\\
\rowcolor{bckeep} NVDA & Weekly Monday  & 82\%  & 82\%  & 58\%  & 14.29\%\\
VRT  & Daily hybrid   & 67\%  & 192\% & 67\%  & 14.29\%\\
\rowcolor{bckeep} MU   & Daily hybrid   & 63\%  & 142\% & 63\%  & 14.29\%\\
VST  & Daily hybrid   & 62\%  & 71\%  & 62\%  & 14.29\%\\
\rowcolor{bckeep} TSM  & Daily hybrid   & 57\%  & 82\%  & 57\%  & 14.29\%\\
\midrule
\textbf{Book} & equal weight & \textbf{83\%} & --- & \textbf{75\%} & 100\%\\
\textbf{Book ex-RKLB} & & \textbf{70\%} & --- & \textbf{61\%} & \\
\bottomrule
\end{tabular}
...
floor --- there are now none. Daily figures carry the corrected OU sleeve of \S9 at the 50/50
split. The planning column equals the daily names' full-sample figure because the tilt, which used
to add about 2pp, is now worth nothing; the derivation is in \S5.1.}
\end{table}
...
\midrule
\rowcolor{bckeep} RKLB & 85  & \textbf{0.25} & 0.022147 & 0.033647 & 50 & 0.005\\
TSM  & 77  & \textbf{0.20} & 0.010022 & 0.020893 & 50 & 0.005\\
\rowcolor{bckeep} VST  & 122 & \textbf{0.65} & 0.014498 & 0.052746 & 50 & 0.005\\
VRT  & 80  & \textbf{0.40} & 0.026603 & 0.059499 & 50 & 0.005\\
\rowcolor{bckeep} MU   & 91  & \textbf{0.75} & 0.025000 & 0.029200 & 50 & 0.005\\
\bottomrule
\end{tabular}
\caption{Buffer $k$ (D3) is shown in bold because it changed with the sigma correction of
\S9 --- residual sigma is about a third of the old scale, so the multiplier is re-scaled to match.
Previous values were RKLB 0.2206, TSM 0.3992, VST 0.2386, VRT 0.5679, MU 0.3000. Interest on idle
cash (R2) is 3.14\% for all names. The stop period is a uniform 50
...
Bayes share \textbf{0.50} for all five names, entered flat rather than per name. The share was
0.75 through Revision 4; \S9.5 sets out why it moved. Per-name Bayes shares remain tested and not
supported: training windows selected a Bayes-heavy share for essentially every name, so the
per-name freedom buys nothing a flat share does not already provide.
...
\rowcolor{bckeep} Bayes fraction --- flat & B6 & \textbf{0.50}\\
...
\subsection{How the planning figures are derived}

The planning figure is what to budget on; the verified figure is what the configuration scored.
For the daily names the basis is the full-sample result at the deployed split, plus whatever the
Bayes tilt is worth \emph{out of sample}. Through Revision 4 that tilt was measured at about
$+2$pp and the planning figures were the 50/50 result plus two points. With the sleeve corrected
the tilt is worth nothing (\S9.5), the split is 50/50, and the planning figure is therefore the
50/50 result itself.

Two discounts still apply to the weekly names, and they are why NVDA and AVGO plan well below
what they scored.
...
\textbf{The weekly model ends fully invested.} Each weekly tranche finishes holding, so all of its
terminal value is an open position. It is already valued at the close here; marking it 20\% lower
would take NVDA to roughly 64\% and AVGO to 76\%.

\begin{center}\fcolorbox{bcgold}{bcact}{\parbox{0.95\textwidth}{\vspace{2pt}
{\color{bcnavy}\bfseries The asymmetry between the daily and weekly planning figures}\\[3pt]
The daily names plan at their full-sample figure while the weekly names are discounted to their
parameter neighbourhood. That is deliberate, not an oversight. The weekly parameters were fitted
directly on this sample and sit on a measurably narrow ridge, so the fitted point overstates. The
daily parameters come from the names' own workbooks and have survived repeated re-fitting attempts
unchanged, and the correction that lifted them is a specification fix rather than a search over
values. The two are therefore not exposed to the same fitting risk and are not discounted the
same way.\vspace{2pt}}}\end{center}
...
\section{The OU sigma correction}

\subsection{What was wrong}

The OU sleeve sets its bid a multiple $k$ of a standard deviation below its forecast. That
standard deviation was computed as \texttt{STDEVP} of the last $W$ closes about the window mean
--- the dispersion of the price \emph{level}. On a name with a sustained trend, most of that
dispersion is the trend itself rather than the innovation the discount is supposed to be measured
in.

The consequence runs the wrong way. The buffer inflates precisely when a name is trending
hardest, pushing the bid furthest below the market exactly when the sleeve should be working, so
fills are lost and the fills that do occur are crashes rather than reversions. Measured on the
book, the old scale ran at 8.7\% of price on TSM and 14.1\% on VRT, against innovation scales of
2.7\% and 3.9\%.

\subsection{The correction}

Sigma is now the standard deviation of the fitted AR(1) residuals, using the same window, the
same mean and the same persistence the sheet already computes. In workbook terms, column AZ
becomes
\begin{quote}\footnotesize\ttfamily
=SQRT(SUMPRODUCT((((R1-AX)-AY*(R0-AX))-((AVERAGE(R1)-AX)-AY*(AVERAGE(R0)-AX)))\^{}2)/(\$B\$3-1))
\end{quote}
where \texttt{R1} and \texttt{R0} are the two shifted close ranges the \texttt{SLOPE} in AY
already uses. The mean of the residuals is written as a linear combination of two plain
\texttt{AVERAGE}s, so no cell needs array entry, and \texttt{SUMPRODUCT} keeps it working on every
Excel version. The formula was checked against the Python engine to $5\times10^{-15}$.

Because residual sigma is roughly a third of the old value, $k$ (D3) is re-scaled per name as
shown in \S2. That is a change of units, not a re-fit.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{OU sleeve alone} & \textbf{old sigma} & \textbf{corrected} & \textbf{$k$ old} &
\textbf{$k$ new}\\
\midrule
\rowcolor{bckeep} RKLB & 171\% & \textbf{199\%} & 0.10 & 0.25\\
MU   & 62\%  & \textbf{83\%}  & 0.20 & 0.75\\
\rowcolor{bckeep} VRT  & 68\%  & 70\%           & 0.20 & 0.40\\
TSM  & 32\%  & 34\%           & 0.05 & 0.20\\
\rowcolor{bckeep} VST  & 54\%  & 54\%           & 0.10 & 0.65\\
\bottomrule
\end{tabular}
\caption{Buffer fitted on the training window only in the walk-forward; shown here at the
full-sample fit.}
\end{table}

Across three expanding folds on each of the five daily names the corrected scale beat the old one
in \textbf{10 of 15 folds}, mean $+14.7$pp. At the deployed 75\% Bayes share the book effect is
smaller, because the sleeve carries a quarter of the capital: the five-name mean rises from 64\%
to 71\% from the equation alone and to 77\% with the re-scaled buffers.

\begin{table}[H]\centering\small
\begin{tabular}{lrrr}
\toprule
\textbf{At 75\% Bayes} & \textbf{old sigma} & \textbf{corrected} & \textbf{change}\\
\midrule
\rowcolor{bckeep} RKLB & 101\% & 134\% & $+33$pp\\
VRT  & 54\%  & 66\%  & $+12$pp\\
\rowcolor{bckeep} MU   & 43\%  & 51\%  & $+8$pp\\
VST  & 61\%  & 66\%  & $+5$pp\\
\rowcolor{bckeep} TSM  & 63\%  & 67\%  & $+4$pp\\
\midrule
mean & 64\% & \textbf{77\%} & $+13$pp\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Two things it does not change}

\textbf{Choosing the share adaptively is still not supported.} Selecting the Bayes share on each
training window picks 100\% as its median in every configuration and beats a fixed 75\% in only 8
or 9 folds of 15. The full-sample curve does invert with the corrected sleeve --- it now prefers
0\% Bayes --- but its shape is regime-dependent rather than stable, and acting on it would be
fitting the sample. The share stays flat and fixed.

\textbf{The weekly names do not take the correction.} On NVDA and AVGO it fails out of sample ---
\textbf{2 of 6 folds, mean $-8.2$pp} --- despite helping both in sample. Those are the most
persistently trending names in the book, which is where the argument for the correction predicts
the largest gain, so the mechanism is less clean than the reasoning suggests. Pooled across all
seven names the correction stands at 12 of 21 folds, $+8.2$pp. It is deployed on the five names
whose own evidence supports it and nowhere else. Even at its in-sample best the corrected daily
model gives NVDA 49\% and AVGO 38\% against 82\% and 91\% weekly, so it does not reopen the
choice of model for those two.

\subsection{Why the split moved to 50/50}

Revision 4 asked whether the share should be \emph{chosen} adaptively and answered no. It did not
ask the question the planning basis actually depends on: whether a fixed 75\% still beats a fixed
50/50. Measured directly, with both frozen and scored on unseen sessions across three expanding
folds per name:

\begin{table}[H]\centering\small
\begin{tabular}{lrrr}
\toprule
\textbf{75\% vs 50/50, both frozen} & \textbf{folds won} & \textbf{mean} & \textbf{median}\\
\midrule
\rowcolor{bckeep} old sleeve & 11 / 15 & $+1.7$pp & $+3.8$pp\\
corrected sleeve & 7 / 15 & $-2.1$pp & $-0.0$pp\\
\bottomrule
\end{tabular}
\end{table}

The tilt was worth something against a mis-specified OU sleeve and is worth nothing against a
correct one. A median of $-0.0$pp is a coin flip, so this is not a case \emph{for} 50/50 so much
as the disappearance of the case for departing from it. The decision is worth about 2pp either
way; 50/50 is the neutral choice and is what the book now runs.

\subsection{Where the correction's gain sits}

A gain concentrated in the untested half of the sample would not be worth budgeting on. This one
is not: across the five daily names at 50/50 the correction adds a mean of $+16$pp over the first
half and $\mathbf{+34}$\textbf{pp over the tested half}. It works hardest in the half that was
scored honestly.

\begin{table}[H]\centering\small
\begin{tabular}{lrrrrrr}
\toprule
& \multicolumn{2}{c}{\textbf{First half}} & \multicolumn{2}{c}{\textbf{Tested half}} &
\multicolumn{2}{c}{\textbf{Full}}\\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
\textbf{Name} & old & new & old & new & old & new\\
\midrule
\rowcolor{bckeep} RKLB & 119\% & 162\% & 74\%  & 154\% & 95\% & 158\%\\
VRT  & $-15$\% & $-5$\% & 134\% & 192\% & 42\% & 67\%\\
\rowcolor{bckeep} MU   & $-9$\%  & 7\%   & 138\% & 142\% & 48\% & 63\%\\
VST  & 51\%  & 53\%  & 53\%  & 71\%  & 52\% & 62\%\\
\rowcolor{bckeep} TSM  & 29\%  & 34\%  & 71\%  & 82\%  & 49\% & 57\%\\
\bottomrule
\end{tabular}
\caption{Daily names at the 50/50 split, verified and marked to market.}
\end{table}

\section{Risks and the 50\% floor}

\textbf{RKLB now dominates the planning figure.} At 158\% it contributes 23pp of the book's
75\%, and the book plans at 61\% without it. That is the number to work from: it is the honest
comparison against Revision 3's 53\%, and it does not depend on one name continuing to behave as
it has. RKLB's tested half of 154\% against a first half of 162\% is at least stable, unlike the
sharp decline it showed under the old sleeve.

\textbf{The planning figures moved a long way in one revision.} From 59\% to 75\% at the book
level is a large jump to take on the strength of a single specification change, however well
validated. The framework has always been built for \emph{ranking} names and sleeves with a safety
margin rather than for absolute forecasting, and that has not changed. A reasonable way to hold
these numbers is that the correction is worth roughly 8pp on a name-typical basis, with RKLB an
outlier that should not be extrapolated.
